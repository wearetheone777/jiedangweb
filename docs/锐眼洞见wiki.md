...
锐安动检（锐眼洞见）是北京锐安科技有限公司开发的移动端 AI 安全检测应用，基于 UniApp 跨平台框架，支持微信小程序、H5 和 App 多端运行。应用提供 AI 安全服务（文本检测、图片检测、音频检测、视频检测）和经典安全服务两大类，采用按次付费 + 免费试用的商业模式，后端服务域名为 `scaneye.cn`。
